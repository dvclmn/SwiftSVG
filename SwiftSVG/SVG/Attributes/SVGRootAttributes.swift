...
///
/// This value deliberately keeps the SVG user-coordinate rectangle (``SVGLayer/viewBox``)
/// separate from the intended layout size (``SVGLayer/viewportSize``).
///
/// Renderer layer geometry is an implementation detail and should not be
/// used as a substitute for either value.
public struct SVGRootAttributes: Equatable, Sendable {
  ...
  static let recognisedAttributeNames = AttributeName.recognised
...
extension SVGRootAttributes {
  ...
  private init(
    parsing attributes: [String: String],
    namespace: String?,
    xlinkNamespace: String?,
  ) {
    self.init(
      width: attributes[AttributeName.width].map(SVGLength.init(rawValue:)),
      height: attributes[AttributeName.height].map(SVGLength.init(rawValue:)),
      viewBox: attributes[AttributeName.viewBox].flatMap(Self.parseViewBox),
      version: attributes[AttributeName.version],
      namespace: namespace,
      xlinkNamespace: xlinkNamespace,
      preserveAspectRatio: attributes[AttributeName.preserveAspectRatio],
    )
  }
...
//  }
  public init(attributes: [String: String]) {
    self.init(
      parsing: attributes,
      namespace: attributes[AttributeName.xmlns],
      xlinkNamespace: attributes[AttributeName.xmlnsXLink],
    )
  }
  ...
  init(
    ...
    self.init(
      parsing: attributes,
      namespace: elementNamespaceURI ?? attributes[AttributeName.xmlns],
      xlinkNamespace: xlinkNamespaceURI ?? attributes[AttributeName.xmlnsXLink],
    )
  }
  //  init(
//    attributes: [String: String],
