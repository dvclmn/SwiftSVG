  ...
  func transform(_ transformString: String) {
    ...
    do {
      let regex = try NSRegularExpression(
        pattern: TransformableConstants.attributesRegex, options: .caseInsensitive,)
      let matches = regex.matches(
        in: transformString, options: [], range: NSMakeRange(0, transformString.utf8.count),)
      ...
      let combinedTransforms =
        ...
        .compactMap({ (thisMatch) -> Transform? in
          ...
          let coordinateString = transformString[
            coordinateRange.location..<coordinateRange.location + coordinateRange.length]
          return Transform(rawValue: transformName, coordinatesString: coordinateString)
        })
        .reduce(
          CGAffineTransform.identity,
          { (accumulate, next) -> CGAffineTransform in
